...
#include <stdlib.h>
#include "..\library.h"
#if defined(_LCD_M0753_) //endif to end of file
/*===========================================================================*/
#ifdef _DT03M_RF_
...
void lcd_display_digit(unsigned char value,unsigned char  pos)
{
    ...
    if (value == DISPLAY_OBIS_CHARACTER_C)
    {
        if ((pos < 9) || (pos == 17)) data = swap4bit(CHAR_C);
        else data = CHAR_C;
    }
    else if (value == '-')//DISPLAY_DASH)
    {
        if ((pos < 9) || (pos == 17)) data = swap4bit(CHAR_DASH);
        else data = CHAR_DASH;
    }
    else if (value < 10) //hien thi so
    {
        if ((pos < 9) || (pos == 17)) data = lcd_seg_data2[value];
